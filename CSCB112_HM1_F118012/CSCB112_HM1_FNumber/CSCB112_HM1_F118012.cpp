...
int main()
{
	// Допълнение 1: Използвам SetConsoleOutputCP(CP_UTF8), за да може кирилицата да се принтира правилно в конзолата локално.
	// Допълнение 2: Използвам std::cout << std::endl, за да добавя нов ред след края на всички принтирания на текст в конзолата за дадената задача, за по-добра четимост.
	// Допълнение 3: При използване на онлайн компилатор или някакъв проблем при локално тестване се премахват редове номер 2 и 10, а именно: #include <windows.h> и SetConsoleOutputCP(CP_UTF8);
    ...
    // Задача 3
    std::cout << "Ключови думи в C++:" << std::endl;
    ...
    std::cout << "Лексема: Най-малката част от синтаксиса на езика, като ключови думи, оператори и идентификатори или константи, която компилаторът разпознава." << std::endl;
    ...
    std::cout << "#include: Включва съдържанието на друг файл в кода. Използва се за добавяне на библиотеки или други файлове." << std::endl;
